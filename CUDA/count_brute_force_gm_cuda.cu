...
int main(int argc, char *argv[]) {
	...
	if (argc < 6) {
		fprintf(stderr, "usage: %s <u> <a> <b> <brute rounds N> <sum N>\n"
	...
	int each_run = B_size / rounds, 
			last_run = B_size / rounds + B_size % rounds;
	...
	err = cudaGetLastError();
	if(err != cudaSuccess)
		fprintf(stderr, "Last intersect %d: %s.\n", i, cudaGetErrorString( err) );
	...
	unsigned int *R_all = 
		(unsigned int *) malloc( (A_size) * sizeof(unsigned int));
	cudaMemcpy(R_all, R_d, A_size * sizeof(unsigned int), cudaMemcpyDeviceToHost);
	for(i = 0; i < A_size; i++)
		printf("%u\n", R_all[i]);
	*/
